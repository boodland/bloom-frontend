...
describe('Welcome badoo page should display', () => {
  ...
  it('about the program content', () => {
    ...
    cy.checkLink('/en/partnership/badoo', 'About the partnership');
  });
  ...
  describe('for a non-logged in user', () => {
    ...
    it('get started panel', () => {
      ...
      cy.get('label[id="accessCode-label"]')
        .should('exist')
        .should('have.attr', 'for', 'accessCode');
      cy.get(inputAccessCodeTag).should('exist');
    });
    describe('navigate to register page when clicking get started action', () => {
      it('without action', () => {
        const registerPartnerBadooPage = getLocalePageUrl('auth/register?partner=badoo');
        cy.get('button').contains('Get started').click();
        cy.url().should('equal', registerPartnerBadooPage);
      });
      it('with action code', () => {
        const actionCode = 'actionCode';
        const registerPartnerBadooPage = getLocalePageUrl(
          `auth/register?partner=badoo&code=${actionCode}`,
        );
  ...
  describe('for a public logged in user', () => {
    ...
    it('continue to bloom panel', () => {
      ...
      cy.checkLink('/en/courses', 'Go to courses');
    });
    after(() => {
      cy.logout();
    });
